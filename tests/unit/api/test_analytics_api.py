...
with patch.dict(
    ...
    from src.python.api.main import _flatten_value_map, app, get_graph_connection
...
class TestUBODiscoverEndpoint:
    ...
    def test_ubo_discover_returns_404_for_missing_company(self, client):
        ...
        response = client.post("/api/v1/ubo/discover", json={"company_id": fake_company_id})
...
class TestRequestValidation:
    ...
    def test_ubo_discover_rejects_invalid_threshold(self, client):
        ...
        response = client.post(
            "/api/v1/ubo/discover",
            json={"company_id": "COMP-001", "ownership_threshold": 150.0},  # Invalid - over 100
        )
    ...
    def test_ubo_discover_rejects_invalid_max_depth(self, client):
        ...
        response = client.post(
            "/api/v1/ubo/discover",
            json={"company_id": "COMP-001", "max_depth": 0},  # Invalid - must be >= 1
        )
    ...
    def test_structuring_rejects_invalid_time_window(self, client):
        ...
        response = client.post(
            "/api/v1/aml/structuring", json={"time_window_days": 0}  # Invalid - must be >= 1
        )
...
class TestErrorHandling:
    ...
    @pytest.mark.skip(
        reason="Difficult to mock with gremlin_python mocked at import time - would require dependency injection refactor"
    )
