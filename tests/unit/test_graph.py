...
try:
    ...
    # Check schema
    mgmt_result = gc.submit('mgmt = graph.openManagement(); labels = mgmt.getVertexLabels(); labelNames = []; labels.each { labelNames.add(it.name()) }; mgmt.rollback(); labelNames').all().result()
