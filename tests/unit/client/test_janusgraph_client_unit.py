...
from src.python.client.exceptions import ConnectionError, QueryError, TimeoutError
from src.python.client.exceptions import ValidationError as ClientValidationError
...
class TestJanusGraphClientQueryExecution:
    ...
    def test_execute_query_with_bindings(self, mock_client_class, monkeypatch):
        ...
        mock_client_instance.submit.assert_called_once_with("g.V().has('name', name)", bindings)
    ...
    def test_execute_query_error(self, mock_client_class, monkeypatch):
        ...
        mock_client_instance.submit.side_effect = GremlinServerError(
            {"code": 500, "message": "Syntax error", "attributes": {}}
        )
        mock_client_class.return_value = mock_client_instance
