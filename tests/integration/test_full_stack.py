...
class TestJanusGraphOperations:
    ...
    def test_create_vertex(self, janusgraph_connection, test_data_cleanup):
        ...
        count = g.V().hasLabel("test_person").has("name", "Integration Test User").count().next()
    ...
    def test_create_edge(self, janusgraph_connection, test_data_cleanup):
        ...
        # Create two vertices
        v1 = g.addV("test_person").property("name", "Person A").next()

        v2 = g.addV("test_person").property("name", "Person B").next()
    ...
    def test_query_traversal(self, janusgraph_connection, test_data_cleanup):
        ...
        friends_of_friends = g.V(alice).out("knows").out("knows").values("name").toList()
    ...
    def test_batch_operations(self, janusgraph_connection, test_data_cleanup):
        ...
        for i in range(vertex_count):
            g.addV("test_entity").property("index", i).property("batch_id", "test_batch_1").next()

        # Verify all vertices were created
        count = g.V().hasLabel("test_entity").has("batch_id", "test_batch_1").count().next()
...
class TestPerformance:
    ...
    def test_bulk_insert_performance(self, janusgraph_connection, test_data_cleanup):
        ...
        for i in range(vertex_count):
            g.addV("perf_test").property("index", i).property("timestamp", int(time.time())).next()
    ...
    def test_traversal_performance(self, janusgraph_connection, test_data_cleanup):
        ...
        for _ in range(iterations):
            result = g.V(vertices[0]).out("next").out("next").out("next").count().next()
            assert result == 1
...
class TestDataPersistence:
    ...
    def test_data_persistence(self, janusgraph_connection, test_data_cleanup):
        ...
        g.addV("persistence_test").property("test_id", test_id).property(
            "created_at", int(time.time())
        ).next()

        # Verify it exists
        count = g.V().hasLabel("persistence_test").has("test_id", test_id).count().next()
    ...
    def test_property_update_persistence(self, janusgraph_connection, test_data_cleanup):
        ...
        vertex = g.addV("persistence_test").property("test_status", "original").next()
...
class TestErrorHandling:
    ...
    def test_concurrent_operations(self, janusgraph_connection, test_data_cleanup):
        ...
        vertex_ids = []
        for i in range(10):
            v = g.addV("test_entity").property("concurrent_id", i).next()
            vertex_ids.append(v.id)

        # Verify all were created
        count = g.V().hasLabel("test_entity").has("concurrent_id").count().next()
