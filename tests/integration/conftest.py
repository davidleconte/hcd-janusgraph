...
def check_service_health(service_name: str) -> Dict[str, any]:
    ...
    if service_name not in SERVICES:
        return {"available": False, "message": f"Unknown service: {service_name}", "details": None}
    ...
    if check_type == "tcp":
        ...
        message = (
            ...
            if available
            else f"{service['name']} is not available on {service['host']}:{service['port']}"
        )
        ...
        message = (
            ...
            if available
            else f"{service['name']} is not responding at {service['url']}"
        )
    else:
        available = False
        message = f"Unknown check type: {check_type}"

    return {"available": available, "message": message, "details": service}
...
def require_hcd(service_health_check):
    ...
    if not health["available"]:
        pytest.skip(f"HCD not available: {health['message']}\n" f"{get_deployment_instructions()}")
    return health
...
def log_test_environment():
    ...
    missing_services = [
        SERVICES[s]["name"] for s in required_services if not check_service_health(s)["available"]
    ]
