...
def gremlin_client():
    ...
    c = client.Client(
        "ws://localhost:18182/gremlin", "g", message_serializer=serializer.GraphSONSerializersV3d0()
    )
    yield c
    c.close()
...
class TestHCDKeyspaceVerification:
    ...
    def test_janusgraph_tables_exist(self, cassandra_session):
        ...
        expected_tables = ["edgestore", "graphindex", "system_properties", "systemlog", "txlog"]
    ...
    def test_edgestore_has_data(self, cassandra_session):
        ...
        result = cassandra_session.execute("SELECT COUNT(*) FROM janusgraph.edgestore LIMIT 1")
...
class TestOpenSearchVectorIndexes:
    ...
    def test_index_document_with_embedding(self, opensearch_client):
        ...
        result = opensearch_client.index(index=index_name, id=test_id, body=doc, refresh=True)
    ...
    def test_knn_search(self, opensearch_client):
        ...
        search_body = {"size": 3, "query": {"knn": {"embedding": {"vector": query_vector, "k": 3}}}}
...
class TestVectorConsumerIntegration:
    ...
    def test_vector_consumer_initialization(self):
        ...
        from banking.streaming.vector_consumer import PULSAR_AVAILABLE, VectorConsumer

        if not PULSAR_AVAILABLE:
            pytest.skip("pulsar-client not available")

        consumer = VectorConsumer(
            pulsar_url="pulsar://localhost:6650", opensearch_host="localhost", opensearch_port=9200
        )
...
class TestFullE2EPipeline:
    ...
    def test_generator_to_janusgraph_via_pulsar(self, gremlin_client, temp_output_dir):
        ...
        with StreamingOrchestrator(config) as orchestrator:
            ...
            logger.info(
                "Generated: %s persons, " f"%s accounts, " f"%s transactions",
    ...
    def test_cross_system_entity_consistency(self, gremlin_client, opensearch_client):
        ...
        jg_persons = (
            gremlin_client.submit("g.V().hasLabel('Person').values('entity_id').limit(5)")
            .all()
            .result()
        )
