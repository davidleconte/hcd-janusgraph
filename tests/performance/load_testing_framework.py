...
class LoadTestingFramework:
    ...
    def run_scenario(
        self, scenario: LoadTestScenario, duration: int = 60, concurrent_users: int = 10
    ) -> PerformanceMetrics:
        ...
        def test_sanitization():
            ...
            try:
                ...
                response = self.session.get(
                    f"{self.base_url}/api/v1/persons/search", params={"name": query}, timeout=5
                )
    ...
    def _test_concurrent_users(self, duration: int, concurrent_users: int) -> PerformanceMetrics:
        ...
        def simulate_user_session():
            ...
            try:
                ...
                # Perform operations
                operations = ["/api/v1/persons", "/api/v1/accounts", "/api/v1/transactions"]

                for op in operations:
                    start = time.time()
                    response = self.session.get(f"{self.base_url}{op}", timeout=5)
                    elapsed = (time.time() - start) * 1000
                    metrics.response_times.append(elapsed)
    ...
    def _test_full_stack(self, duration: int, concurrent_users: int) -> PerformanceMetrics:
        ...
        for scenario in scenarios:
            scenario_metrics = self.run_scenario(
                scenario, duration // len(scenarios), concurrent_users
            )
...
def main():
    ...
    metrics = framework.run_scenario(
        scenario, duration=args.duration, concurrent_users=args.concurrent_users
    )
