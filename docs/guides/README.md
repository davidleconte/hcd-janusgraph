# User and Developer Guides

This directory contains comprehensive guides for users, developers, and operators of the HCD + JanusGraph banking compliance system.

**Date:** 2026-01-28
**Status:** Active

## Available Guides

### Setup and Configuration

#### [Setup Guide](setup-guide.md)

Complete installation and configuration guide for development and production environments.

**Topics Covered:**

- Prerequisites and system requirements
- Installation steps
- Configuration options
- Environment setup
- Initial data loading
- Verification procedures

**Audience:** Developers, Operators

---

### Testing and Quality Assurance

#### [Testing Guide](testing-guide.md)

Comprehensive testing strategies and execution procedures.

**Topics Covered:**

- Unit testing
- Integration testing
- Performance testing
- Test execution
- Coverage reporting
- CI/CD integration

**Audience:** Developers, QA Engineers

---

### Deployment and Operations

#### [Deployment Guide](deployment-guide.md)

Production deployment procedures and best practices.

**Topics Covered:**

- Deployment strategies
- Environment preparation
- Service deployment
- Health checks
- Rollback procedures
- Post-deployment validation

**Audience:** Operators, DevOps Engineers

---

### Troubleshooting and Support

#### [Troubleshooting Guide](troubleshooting-guide.md)

Common issues, solutions, and debugging procedures.

**Topics Covered:**

- Common problems and solutions
- Debugging techniques
- Log analysis
- Performance issues
- Connection problems
- Data issues

**Audience:** All Users

---

## Quick Navigation

### By Role

**Developers:**

- [Setup Guide](setup-guide.md) - Get started with development
- [Testing Guide](testing-guide.md) - Write and run tests
- [Troubleshooting Guide](troubleshooting-guide.md) - Debug issues

**Operators:**

- [Deployment Guide](deployment-guide.md) - Deploy to production
- [Troubleshooting Guide](troubleshooting-guide.md) - Resolve operational issues
- [Operations Runbook](../operations/operations-runbook.md) - Day-to-day operations

**QA Engineers:**

- [Testing Guide](testing-guide.md) - Testing procedures
- [Setup Guide](setup-guide.md) - Test environment setup

### By Task

**Getting Started:**

1. [Setup Guide](setup-guide.md) - Install and configure
2. [Testing Guide](testing-guide.md) - Verify installation
3. [Troubleshooting Guide](troubleshooting-guide.md) - Fix any issues

**Deploying to Production:**

1. [Deployment Guide](deployment-guide.md) - Deployment procedures
2. [Operations Runbook](../operations/operations-runbook.md) - Operational procedures
3. [Monitoring Guide](../operations/monitoring-guide.md) - Set up monitoring

**Troubleshooting:**

1. [Troubleshooting Guide](troubleshooting-guide.md) - Common issues
2. [Operations Runbook](../operations/operations-runbook.md) - Operational issues
3. [Testing Guide](testing-guide.md) - Test-related issues

## Related Documentation

### Operations

- [Operations Runbook](../operations/operations-runbook.md)
- [Monitoring Guide](../operations/monitoring-guide.md)
- [Backup Procedures](../operations/backup-procedures.md)
- [Disaster Recovery](../operations/disaster-recovery-plan.md)

### Development

- [Contributing Guidelines](../CONTRIBUTING.md)
- Code Refactoring Guide
- [API Reference](../banking/guides/api-reference.md)

### Architecture

- [System Architecture](../architecture/system-architecture.md)
- [Architecture Decision Records](../architecture/)

## Support

For additional help:

1. Check the relevant guide above
2. Review [Troubleshooting Guide](troubleshooting-guide.md)
3. Consult [Documentation Index](../index.md)
4. Check [Operations Runbook](../operations/operations-runbook.md)

## Contributing

To improve these guides:

1. Follow [Documentation Standards](../documentation-standards.md)
2. Use clear, concise language
3. Include examples and screenshots
4. Test all procedures
5. Submit updates via pull request

See [Contributing Guidelines](../CONTRIBUTING.md) for details.
