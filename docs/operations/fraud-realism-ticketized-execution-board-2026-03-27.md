...
| FR-043 | P2 | KPI trend aggregation and weekly evidence rollup 🚧 IN PROGRESS (2026-03-29) | Platform + Domain | M | FR-042 | Cross-run KPI trend artifact + governance evidence rollup | G-API, G-NBK | Step 1 delivered: added `scripts/testing/aggregate_kpi_trends.py`, unit tests `tests/unit/test_aggregate_kpi_trends.py` (3 passed), and repeatable pipeline integration step `KPI Trend Aggregation` in `scripts/testing/run_demo_pipeline_repeatable.sh`; validation: `pytest tests/unit/test_aggregate_kpi_trends.py tests/unit/test_detect_kpi_drift.py -v --no-cov` (7 passed). |
