...
| FR-045 | P2 | Case evidence PDF exporter 🚧 IN PROGRESS (2026-03-29) | Platform + Compliance | M | FR-044 | Deterministic PDF rendering + optional bundle inclusion | G-API, G-NBK | Step 1 delivered: new `scripts/reporting/generate_case_pdf.py`, optional `--pdf` mode in `scripts/testing/bundle_governance_evidence.py`, runbook guidance in `docs/operations/operations-runbook.md`, and test coverage (`tests/unit/test_generate_case_pdf.py`, updated `tests/unit/test_bundle_governance_evidence.py`); evidence: `pytest tests/unit/test_generate_case_pdf.py tests/unit/test_bundle_governance_evidence.py -v --no-cov` (6 passed) and `python3 scripts/testing/bundle_governance_evidence.py --pdf` (`files=8`, `pdfs=2`). |
