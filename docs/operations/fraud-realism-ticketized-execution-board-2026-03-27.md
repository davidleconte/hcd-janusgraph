...
| Ticket ID | Priority | Title | Owner | Estimate | Dependencies | Scope | Exact Gate Checks | Done Criteria |
|---|---|---|---|---|---|---|---|---|
| FR-020 | P1 | Sanctions multi-factor weighted scoring 🚧 IN PROGRESS (2026-03-29) | Domain + Compliance | L | FR-012 | Name+DOB+country+association weighted model | G-NBK, G-OS, G-DET | Step 1 delivered in `banking/aml/sanctions_screening.py`: `SanctionMatch` now exposes `weighted_score` + `reason_codes`, and `screen_customer` emits deterministic reason codes (`SANCTION_NAME_MATCH`, `JURISDICTION_MATCH`, `ENTITY_TYPE_MATCH`) under weighted context; validation: `pytest banking/aml/tests/test_sanctions_screening.py -v --no-cov` (22 passed). |
