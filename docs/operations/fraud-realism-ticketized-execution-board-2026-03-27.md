...
| FR-021 | P1 | UBO recursive effective ownership/control rights ✅ DONE (2026-04-02) | Domain | L | FR-002 | Deterministic recursive effective-control logic + circularity API surfacing | G-NBK, G-DET, G-API | Recursive effective ownership + control-rights qualification implemented in `src/python/analytics/ubo_discovery.py` with API-facing circularity signal `has_circular_ownership` now propagated through repository and UBO response (`cc3d778`); evidence: targeted unit/API tests PASS, notebook gate PASS, and deterministic proof run `exports/demo-20260402T101618Z/` (`drift_detection.log` PASS, all notebooks PASS). |
...
| FR-045 | P2 | Case evidence PDF exporter ✅ DONE (2026-04-02) | Platform + Compliance | M | FR-044 | Deterministic PDF rendering + optional bundle inclusion | G-API, G-NBK | FR-045 delivered with deterministic PDF generator `scripts/reporting/generate_case_pdf.py` and optional `--pdf` integration in `scripts/testing/bundle_governance_evidence.py`; upgraded with deterministic multi-page rendering (`1059d80`). Evidence: `pytest tests/unit/test_generate_case_pdf.py tests/unit/test_bundle_governance_evidence.py -v --no-cov` (7 passed), and full deterministic proof run `exports/demo-20260402T101618Z/` (PASS, all notebooks PASS, bundle step PASS). |
