# Banking Architecture Documentation

**Author:** David Leconte, IBM Worldwide | Tiger-Team, Watsonx.Data Global Product Specialist (GPS)
**Last Updated:** 2026-02-06

## Overview

This directory contains architecture documentation for the Banking module.

## Contents

- **[Architecture](../architecture/README.md)** - Banking module architecture overview
- **Enterprise Advanced Patterns Plan** - Advanced design patterns

## Related Documentation

- [System Architecture](../../architecture/system-architecture.md)
- [Streaming Architecture](../../architecture/streaming-architecture.md)
- [Data Flow](../../architecture/data-flow-unified.md)
