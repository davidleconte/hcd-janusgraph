...
class AMLStructuringDetector:
    ...
    def analyze_transaction_amounts(self) -> Dict[str, Any]:
        ...
        print(
            f"  $9K - $10K: {between_9k_10k} ({between_9k_10k/total_count*100:.1f}%) ⚠️  SUSPICIOUS RANGE"
        )
        print(f"  Above $10K: {above_10k} ({above_10k/total_count*100:.1f}%)")

        if between_9k_10k > 0:
            print(f"\n🚨 ALERT: {between_9k_10k} transactions in suspicious range ($9K-$10K)")
            print("   These may indicate structuring to avoid CTR reporting!")
    ...
    def identify_high_volume_accounts(self) -> List[Dict[str, Any]]:
        ...
        for r in results[:10]:
            ...
            if total > 5:  # Significant activity
                ...
                print(
                    f"  {account_id[:20]}... | Sent: {sent} | Received: {received} | Total: {total}"
                )
        ...
        try:
            ...
            for r in results[:10]:
                if isinstance(r, dict):
                    ...
                    chains.append(chain)
                    print(
                        f"  {chain['source'][:15]}... -> {chain['intermediate'][:15]}... -> {chain['destination'][:15]}..."
                    )
