...
class StreamingOrchestrator(MasterOrchestrator):
    ...
    def _publish_entity(self, entity: Any, event_type: str = "create") -> bool:
        ...
        try:
            event = convert_entity_to_event(
                entity, event_type=event_type, source="StreamingOrchestrator"
            )
    ...
    def _generate_core_entities(self):
        ...
        for i in range(self.config.person_count):
            ...
            if (i + 1) % 100 == 0:
                logger.info(
                    "  Generated and published %d/%d persons", i + 1, self.config.person_count
                )
        ...
        for i in range(self.config.company_count):
            ...
            if (i + 1) % 10 == 0:
                logger.info(
                    "  Generated and published %d/%d companies", i + 1, self.config.company_count
                )
        ...
        for i in range(self.config.account_count):
            if random.random() < 0.8:  # 80% person accounts
                ...
                account = self.account_gen.generate(owner_id=owner.id, owner_type="person")
            else:  # 20% company accounts
                owner = random.choice(self.companies)
                account = self.account_gen.generate(owner_id=owner.id, owner_type="company")
            self.accounts.append(account)
            self._publish_entity(account)

            if (i + 1) % 100 == 0:
                logger.info(
                    "  Generated and published %d/%d accounts", i + 1, self.config.account_count
                )
    ...
    def _generate_events(self):
        ...
        if self.config.transaction_count > 0:
            ...
            for i in range(self.config.transaction_count):
                ...
                if (i + 1) % 1000 == 0:
                    logger.info(
                        "  Generated and published %d/%d transactions",
                        i + 1,
        ...
        if self.config.communication_count > 0:
            ...
            for i in range(self.config.communication_count):
                ...
                if (i + 1) % 1000 == 0:
                    logger.info(
                        "  Generated and published %d/%d communications",
                        i + 1,
        ...
        if self.config.trade_count > 0:
            ...
            for i in range(self.config.trade_count):
                ...
                if (i + 1) % 100 == 0:
                    logger.info(
                        "  Generated and published %d/%d trades", i + 1, self.config.trade_count
                    )
        ...
        if self.config.travel_count > 0:
            ...
            for i in range(self.config.travel_count):
                ...
                if (i + 1) % 100 == 0:
                    logger.info(
                        "  Generated and published %d/%d travel records",
                        i + 1,
                        self.config.travel_count,
                    )
        ...
        if self.config.document_count > 0:
            ...
            for i in range(self.config.document_count):
                ...
                if (i + 1) % 100 == 0:
                    logger.info(
                        "  Generated and published %d/%d documents",
                        i + 1,
