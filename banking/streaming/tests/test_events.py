...
class TestEntityEvent:
    ...
    def test_invalid_event_type(self):
        ...
        with pytest.raises(ValueError, match="Invalid event_type"):
            EntityEvent(
                entity_id="test-123", event_type="invalid", entity_type="person", payload={}
            )
    ...
    def test_invalid_entity_type(self):
        ...
        with pytest.raises(ValueError, match="Invalid entity_type"):
            EntityEvent(
                entity_id="test-123", event_type="create", entity_type="invalid", payload={}
            )
    ...
    def test_get_topic(self):
        ...
        event = EntityEvent(
            entity_id="test-123", event_type="create", entity_type="person", payload={}
        )

        assert event.get_topic() == "persistent://public/banking/persons-events"

        event2 = EntityEvent(
            entity_id="test-456", event_type="create", entity_type="account", payload={}
        )
...
class TestEntityEventBatch:
    ...
    def test_batch_iteration(self):
        ...
        events = [
            EntityEvent(
                entity_id=f"test-{i}", event_type="create", entity_type="person", payload={}
            )
            for i in range(3)
        ]
...
class TestFactoryFunctions:
    ...
    def test_create_account_event(self):
        ...
        event = create_account_event(account_id="acc-123", payload={"balance": 1000.0})
    ...
    def test_create_transaction_event(self):
        ...
        event = create_transaction_event(
            transaction_id="txn-123", payload={"amount": 500.0, "currency": "USD"}
        )
    ...
    def test_create_company_event(self):
        ...
        event = create_company_event(
            company_id="comp-123", name="ACME Corp", payload={"industry": "Tech"}
        )
