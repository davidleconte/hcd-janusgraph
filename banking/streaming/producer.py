...
try:
    import pulsar
    from pulsar import Client, CompressionType, Producer
...
class EntityProducer:
    ...
    def __init__(
        ...
        if not PULSAR_AVAILABLE:
            raise ImportError(
                "pulsar-client is not installed. " "Install with: pip install pulsar-client>=3.4.0"
            )
    ...
    def _connect(self):
        ...
        try:
            ...
            self.client = pulsar.Client(
                self.pulsar_url, operation_timeout_seconds=30, connection_timeout_ms=10000
            )
