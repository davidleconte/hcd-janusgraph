...
class TestSmurfingPatternAnalysis:
    ...
    def test_analyze_smurfing_high_confidence(self):
        ...
        pattern = detector._analyze_smurfing_pattern(
            account_id='ACC-123',
            transactions=transactions,
            time_window_hours=6  # Short window
        )
    ...
    def test_analyze_smurfing_low_variance(self):
        ...
        assert pattern is not None
        assert any('similar' in ind.lower() or 'variance' in ind.lower() 
...
class TestLayeringPatternAnalysis:
    ...
    def test_analyze_layering_confidence_scaling(self):
        ...
        pattern_few = detector._analyze_layering_pattern(
            'ACC-123', transactions_few, 48
        )
        pattern_many = detector._analyze_layering_pattern(
            'ACC-123', transactions_many, 48
        )
...
class TestNetworkPatternAnalysis:
    ...
    def test_analyze_network_large_network(self):
        ...
        transactions = [
            {'id': f'TX-{i:03d}', 'amount': 9000.0, 'timestamp': 1000000 + i*1000, 
             'account': f'ACC-{i:03d}'}
            for i in range(10)
        ]
        
        pattern_small = detector._analyze_network_pattern(
            account_ids_small, transactions[:3], 24
        )
        pattern_large = detector._analyze_network_pattern(
            account_ids_large, transactions, 24
        )
...
class TestRiskLevelDetermination:
    ...
    def test_risk_level_critical(self):
        ...
        pattern = detector._analyze_smurfing_pattern(
            'ACC-123', transactions, 6  # Short window
        )
