...
class TestMultiHopMoneyLaunderingChains:
    ...
    def test_three_hop_laundering_chain(self, mock_search, mock_gen):
        ...
        # Score each hop
        hop1_score = detector.score_transaction('TX-HOP1', 'ACC-001', 9500.0, 'Wire Transfer', 'Transfer to business')
        hop2_score = detector.score_transaction('TX-HOP2', 'ACC-002', 9300.0, 'Wire Transfer', 'Transfer to partner')
        hop3_score = detector.score_transaction('TX-HOP3', 'ACC-003', 9000.0, 'Wire Transfer', 'Payment to supplier')
...
class TestShellCompanyDetection:
    ...
    def test_layered_shell_company_network(self, mock_search, mock_gen):
        ...
        for from_acc, to_acc, amount in shell_network:
            ...
            score = detector.score_transaction(f'TX-{from_acc}', from_acc, amount, 'Intercompany', 'Transfer')
...
class TestIntegratedDetectionPipeline:
    ...
    def test_alert_generation_for_critical_transactions(self, mock_search, mock_gen):
        ...
        score = detector.score_transaction('TX-CRIT', 'ACC-CRIT', 50000.0, 'Crypto Exchange', 'Large crypto')
