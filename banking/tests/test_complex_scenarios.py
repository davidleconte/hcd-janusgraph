...
class TestCheckMerchantComplexScenarios:
    ...
    def test_crypto_exchange_high_risk(self, mock_search, mock_gen):
        ...
        for merchant in crypto_merchants:
            ...
            assert risk_score >= 0.3, f"Crypto merchant '{merchant}' should have risk >= 0.3, got {risk_score}"
    ...
    def test_gambling_merchant_high_risk(self, mock_search, mock_gen):
        ...
        for merchant in gambling_merchants:
            risk_score = detector._check_merchant(merchant)
            assert risk_score >= 0.3, f"Gambling merchant '{merchant}' should have risk >= 0.3, got {risk_score}"
    ...
    def test_normal_merchant_low_risk(self, mock_search, mock_gen):
        ...
        for merchant in normal_merchants:
            risk_score = detector._check_merchant(merchant)
            assert risk_score < 0.3, f"Normal merchant '{merchant}' should have risk < 0.3, got {risk_score}"
    ...
    def test_merchant_with_fraud_history(self, mock_search, mock_gen):
        ...
        assert risk_score > 0.2, f"Merchant similar to fraud cases should have elevated risk"
...
class TestCheckBehaviorComplexScenarios:
    ...
    def test_new_account_first_transaction(self, mock_conn, mock_search, mock_gen):
        ...
        mock_g.V.return_value.has.return_value.outE.return_value.has.return_value.project.return_value.by.return_value.by.return_value.by.return_value = mock_traversal
        ...
        with patch('banking.fraud.fraud_detection.traversal') as mock_traversal_func:
            ...
            assert 0.2 <= risk_score <= 0.5, f"First transaction should have moderate risk"
    ...
    def test_unusual_amount_deviation(self, mock_conn, mock_search, mock_gen):
        ...
        mock_traversal.toList.return_value = [
            {'amount': [50, 60, 75, 80, 45, 90, 55, 70], 'merchant': ['Store'] * 8, 'description': ['Purchase'] * 8}
        ]
        mock_g.V.return_value.has.return_value.outE.return_value.has.return_value.project.return_value.by.return_value.by.return_value.by.return_value = mock_traversal
        mock_conn.return_value.__enter__ = Mock(return_value=mock_g)
        mock_conn.return_value.__exit__ = Mock(return_value=False)
...
class TestDetectSemanticPatternsComplexScenarios:
    ...
    def test_detect_semantically_similar_transactions(self, mock_conn, mock_search, mock_gen):
        ...
        mock_traversal.toList.return_value = [
            {
                'tx_id': 'TX-001', 'amount': 9500.0, 'description': 'Wire transfer to business',
                'merchant': 'Bank', 'account_id': 'ACC-001', 'person_id': ['P-001'], 
                'person_name': ['John Doe'], 'timestamp': 1000000
            },
            {
                'tx_id': 'TX-002', 'amount': 9600.0, 'description': 'Wire transfer to business account',
                'merchant': 'Bank', 'account_id': 'ACC-002', 'person_id': ['P-001'], 
                'person_name': ['John Doe'], 'timestamp': 1001000
            },
            {
                'tx_id': 'TX-003', 'amount': 9400.0, 'description': 'Business wire transfer',
                'merchant': 'Bank', 'account_id': 'ACC-003', 'person_id': ['P-001'], 
                'person_name': ['John Doe'], 'timestamp': 1002000
            },
        ]
        mock_g.V.return_value.hasLabel.return_value.has.return_value.has.return_value.has.return_value.project.return_value.by.return_value.by.return_value.by.return_value.by.return_value.by.return_value.by.return_value.by.return_value.by.return_value.limit.return_value = mock_traversal
