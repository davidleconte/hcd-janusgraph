...
def get_gremlin_client():
    ...
    return client.Client(JANUSGRAPH_CONFIG["url"], JANUSGRAPH_CONFIG["traversal_source"])
...
def init_notebook(check_env: bool = True, check_services: bool = False):
    ...
    result = {"project_root": setup_project_paths()}
    ...
    if check_services:
        ...
        try:
            ...
            os_client.info()
            print(
                f"✅ OpenSearch connected at {OPENSEARCH_CONFIG['host']}:{OPENSEARCH_CONFIG['port']}"
            )
