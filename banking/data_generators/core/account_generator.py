...
from ..utils.data_models import Account, AccountType, RiskLevel
...
class AccountGenerator(BaseGenerator[Account]):
    ...
    def generate(self, owner_id: Optional[str] = None, owner_type: str = "person") -> Account:
        """
    ...
    def _generate_status(self) -> str:
        ...
        choices = [("active", 0.85), ("dormant", 0.08), ("frozen", 0.02), ("closed", 0.05)]
        return random_choice_weighted(choices)
    ...
    def _generate_transaction_totals(
        self, transaction_count: int, current_balance: Decimal
    ) -> tuple:
        ...
        total_deposits = current_balance + random_amount(
            float(current_balance) * 0.5, float(current_balance) * 5.0
        )
    ...
    def _calculate_average_balance(
        self, current_balance: Decimal, total_deposits: Decimal, total_withdrawals: Decimal
    ) -> Decimal:
