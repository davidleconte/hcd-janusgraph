...
def random_amount(min_amount: float, max_amount: float, 
                 round_probability: float = 0.3) -> Decimal:
    """
...
def random_just_below_threshold(country_code: str = "US", 
                                margin_percent: float = 0.05) -> Decimal:
    """
...
def is_just_below_threshold(amount: Decimal, country_code: str = "US",
...
def contains_suspicious_keywords(text: str, category: Optional[str] = None) -> Tuple[bool, List[str]]:
    """
...
def detect_structuring_pattern(transactions: List[Dict[str, Any]], 
...
def calculate_pattern_confidence(indicators: List[str], 
                                 red_flags: List[str],
                                 total_value: Decimal,
