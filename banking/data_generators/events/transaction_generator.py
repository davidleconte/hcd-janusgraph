...
from ..utils.helpers import (
    ...
    random_business_hours_datetime, random_amount,
    random_just_below_threshold, is_round_amount,
    is_just_below_threshold, calculate_transaction_risk_score
)
...
class TransactionGenerator(BaseGenerator[Transaction]):
    ...
    def generate(
        ...
        if is_structuring:
            amount = random_just_below_threshold(
                self._get_country_from_currency(currency)
            )
        else:
            amount = random_amount(
                self.min_amount,
                self.max_amount,
                round_probability=self.round_amount_probability
            )
        ...
        risk_score = calculate_transaction_risk_score(
            ...
            involves_tax_haven=(
                originating_country in self._get_tax_havens() or
                destination_country in self._get_tax_havens()
            )
        )
        
        is_suspicious = (
            risk_score > 0.5 or
            random.random() < self.suspicious_probability
        )
    ...
    def _generate_merchant_category(self, txn_type: TransactionType) -> Optional[str]:
        ...
        if txn_type in [TransactionType.POS, TransactionType.ONLINE, TransactionType.PAYMENT]:
            categories = [
                ...
                "airline", "entertainment", "healthcare", "utilities", "insurance"
            ]
    ...
    def _generate_pattern_type(self) -> str:
        ...
        patterns = [
            "structuring", "layering", "rapid_movement", "circular",
            "smurfing", "cuckoo_smurfing", "trade_based", "round_tripping"
        ]
