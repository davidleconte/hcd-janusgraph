...
class CommunicationGenerator(BaseGenerator[Communication]):
    ...
    def generate(
        ...
        if communication_type is None:
            communication_type = random_choice_weighted(
                list(self.type_weights.items())
            )
        ...
        if language is None:
            ...
            lang_weights = [(code, 0.40 if code == "en" else 0.60 / (len(lang_codes) - 1))
        ...
        # Generate sentiment
        sentiment = random_choice_weighted(
            list(self.sentiment_weights.items())
        )
    ...
    def _get_faker_locale(self, language_code: str) -> str:
        ...
        locale_map = {
            ...
            "ja": "ja_JP", "ko": "ko_KR", "ar": "ar_SA", "hi": "hi_IN"
        }
    ...
    def _generate_metadata(
        ...
        if comm_type == CommunicationType.EMAIL:
            metadata.update({
                ...
                "in_reply_to": f"<{self.faker.uuid4()}@{self.faker.domain_name()}>" if random.random() < 0.4 else None
            })
