...
class DocumentGenerator(BaseGenerator[Document]):
    ...
    def __init__(
        ...
        self.product_categories = [
            ...
            "Pharmaceuticals", "Automotive Parts", "Food Products",
            "Raw Materials", "Consumer Goods", "Industrial Equipment"
        ]
    ...
    def generate(
        ...
        if document_type is None:
            document_type = random_choice_weighted(
                list(self.document_type_weights.items())
            )
        ...
        currency = random_choice_weighted([
            ("USD", 0.50), ("EUR", 0.20), ("GBP", 0.10),
            ("CNY", 0.08), ("JPY", 0.05), ("CHF", 0.07)
        ])
        ...
        total_amount = sum(Decimal(str(item["amount"])) for item in line_items) if line_items else Decimal("0.00")
    ...
    def _generate_metadata(
        ...
        metadata: Dict[str, Any] = {
            "document_type": document_type,
            "currency": currency,
            "payment_terms": random.choice(["Net 30", "Net 60", "Net 90", "COD", "Advance Payment"]),
