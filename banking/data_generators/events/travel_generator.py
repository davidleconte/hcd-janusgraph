...
class TravelGenerator(BaseGenerator[TravelEvent]):
    ...
    def generate(
        ...
        visa_obtained = True if not visa_required else random.random() < 0.95  # 95% obtain if required
    ...
    def _generate_suspicious_indicators(
        ...
        if duration_days <= 2 and (is_high_risk_country(arrival_country) or is_tax_haven(arrival_country)):
    ...
    def _generate_metadata(
        ...
        if transport_mode == "air":
            metadata["flight_number"] = f"{random.choice(['AA', 'BA', 'LH', 'AF', 'UA'])}{random.randint(100, 9999)}"
            metadata["airline"] = random.choice(["American", "British Airways", "Lufthansa", "Air France", "United"])
