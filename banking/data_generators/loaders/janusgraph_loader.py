...
class JanusGraphLoader:
    ...
    def __init__(
        self,
        url: str = "ws://localhost:18182/gremlin",
    ...
    def load_persons(self, persons: List[Any]) -> Dict[str, int]:
        ...
        for i, person in enumerate(persons):
            ...
            existing = self._submit(
                "g.V().has('person', 'person_id', pid).id()",
                {"pid": person_id}
            )
    ...
    def load_companies(self, companies: List[Any]) -> Dict[str, int]:
        ...
        for i, company in enumerate(companies):
            ...
            existing = self._submit(
                "g.V().has('company', 'company_id', cid).id()",
                {"cid": company_id}
            )
    ...
    def load_accounts(
        ...
        for i, account in enumerate(accounts):
            ...
            existing = self._submit(
                "g.V().has('account', 'account_id', aid).id()",
                {"aid": account_id}
            )
    ...
    def load_transactions(
        ...
        for i, tx in enumerate(transactions):
            ...
            existing = self._submit(
                "g.V().has('transaction', 'transaction_id', tid).id()",
                {"tid": tx_id}
            )
    ...
    def load_from_orchestrator(self, orchestrator: Any, clear_first: bool = False):
        ...
        try:
            ...
            transaction_id_map = self.load_transactions(
...
if __name__ == "__main__":
    ...
    # Pattern arguments
    parser.add_argument("--insider-trading", type=int, default=0, help="Number of insider trading patterns")
    ...
    print(f"Loaded: {stats['loading']['vertices_created']} vertices, {stats['loading']['edges_created']} edges")
    print("=" * 80)
