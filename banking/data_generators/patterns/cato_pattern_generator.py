...
from ..utils.data_models import Communication, Pattern, RiskLevel, Transaction
from ..utils.helpers import random_amount, random_date_between
...
class CATOPatternGenerator(BaseGenerator[Pattern]):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if start_date is None:
            ...
            actual_start_date = datetime.combine(
                random_date_between(start, end), datetime.min.time()
            )
        else:
    ...
    def _generate_credential_stuffing(
        self, victim_count: int, attacker_count: int, start_date: datetime, end_date: datetime
    ) -> Pattern:
        ...
        for account in victim_accounts:
            ...
            for _ in range(random.randint(2, 5)):
                ...
                txn = self.transaction_gen.generate(
                    ...
                    to_account_id=self.account_gen.generate().account_number,
                )
                # Override generated values
                txn.amount = amount
                txn.transaction_date = takeover_date + timedelta(minutes=random.randint(5, 30))
        ...
        return Pattern(
            ...
            end_date=end_date,
            entity_ids=[e.person_id for e in victims + attackers],
            entity_types=["person"] * (len(victims) + len(attackers)),
            transaction_ids=[t.transaction_id for t in transactions],
            communication_ids=[c.communication_id for c in communications],
            total_value=total_stolen,
            ...
            metadata={
                ...
                "victim_ids": [v.person_id for v in victims],
                "attacker_ids": [a.person_id for a in attackers],
                "account_ids": [acc.account_number for acc in victim_accounts],
            },
        )
    ...
    def _generate_session_hijacking(
        ...
        for account in victim_accounts:
            ...
            indicators.append("Rapid navigation pattern change detected")
            indicators.append("Typing speed anomaly detected")
            ...
            for _ in range(random.randint(1, 3)):
                ...
                txn.amount = amount
                txn.transaction_date = hijack_time + timedelta(minutes=random.randint(1, 15))
                transactions.append(txn)
    ...
    def _generate_sim_swap(
        ...
        for i, account in enumerate(victim_accounts):
            ...
            comm = self.communication_gen.generate(
                sender_id=attacker.person_id, recipient_id=victim.person_id
            )
            communications.append(comm)

            indicators.append(f"Phone carrier contacted for {victim.full_name}")

            # Phase 2: SIM swap executed
            swap_date = call_date + timedelta(hours=1)
            victim_phone = victim.phone_numbers[0].number if victim.phone_numbers else "Unknown"
            indicators.append(f"SIM card swapped for {victim_phone}")
            red_flags.append(f"Unauthorized SIM swap detected for {victim_phone}")

            # Phase 3: SMS 2FA codes intercepted
            indicators.append("2FA codes redirected to new SIM")
            ...
            for _ in range(random.randint(2, 4)):
                ...
                txn.amount = amount
                txn.transaction_date = takeover_date + timedelta(minutes=random.randint(5, 20))
                transactions.append(txn)
    ...
    def _generate_phishing_campaign(
        ...
        for account in compromised_accounts:
            ...
            for _ in range(random.randint(1, 3)):
                ...
                txn.amount = amount
                txn.transaction_date = exploit_date + timedelta(hours=random.randint(0, 24))
                transactions.append(txn)
    ...
    def _generate_malware_based(
        ...
        for account in victim_accounts:
            ...
            for _ in range(random.randint(2, 5)):
                ...
                txn.amount = amount
                txn.transaction_date = exploit_date + timedelta(hours=random.randint(0, 48))
                transactions.append(txn)
    ...
    def _calculate_confidence_score(
        self, indicator_count: int, red_flag_count: int, victim_count: int, failed_attempts: int
    ) -> float:
    ...
    def _calculate_severity_score(
        self, confidence_score: float, total_value: Decimal, victim_count: int, indicator_count: int
    ) -> float:
    ...
    def _generate_ip_address(self) -> str:
        """Generate a random IP address for attacker infrastructure."""
        # Generate a random IPv4 address
        # Avoid reserved ranges (private, loopback, etc.) for realism
        while True:
