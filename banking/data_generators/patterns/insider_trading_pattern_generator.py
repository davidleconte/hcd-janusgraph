...
class InsiderTradingPatternGenerator(BaseGenerator[Pattern]):
    ...
    def generate(
        ...
        if existing_entity_ids:
            ...
            if sample_count < entity_count:
                entity_ids.extend(
                    [f"PER-{self.faker.uuid4()[:8]}" for _ in range(entity_count - sample_count)]
                )
        else:
        ...
        trades = self._generate_pattern_trades(
            entity_ids, trade_count, start_date, announcement_date, pattern_type
        )

        # Generate communications (if applicable)
        communications = self._generate_pattern_communications(
            entity_ids, start_date, announcement_date, pattern_type
        )
        ...
        indicators = self._generate_pattern_indicators(
            pattern_type, trades, communications, days_before_announcement
        )

        # Generate red flags
        red_flags = self._generate_red_flags(pattern_type, trades, communications, entity_count)
        ...
        severity_score = self._calculate_severity_score(
            confidence_score, total_value, entity_count, days_before_announcement
        )
        ...
        pattern = Pattern(
            ...
            metadata={
                ...
                "insider_relationships": random.sample(
                    self.insider_relationships, k=min(entity_count, 5)
                ),
            },
        )
    ...
    def _generate_pattern_trades(
        ...
        for i in range(trade_count):
            ...
            trader_id = random.choices(
                entity_ids, weights=[1.0 / (j + 1) for j in range(len(entity_ids))], k=1
            )[0]
    ...
    def _generate_pattern_communications(
        self, entity_ids: List[str], start_date: datetime, end_date: datetime, pattern_type: str
    ) -> List[Any]:
        ...
        for i in range(comm_count):
            ...
            comm_date = random_datetime_between(
                start_date, start_date + timedelta(days=(end_date - start_date).days // 2)
            )

            # Generate communication
            comm = self.comm_gen.generate(
                sender_id=sender_id, recipient_id=recipient_id, force_suspicious=True
            )
    ...
    def _generate_pattern_indicators(
        ...
        if pattern_type == "coordinated_insider_trading":
            indicators.extend(
                ["coordinated_trading_pattern", "simultaneous_trades", "identical_trade_direction"]
            )
    ...
    def _generate_red_flags(
        self, pattern_type: str, trades: List[Any], communications: List[Any], entity_count: int
    ) -> List[str]:
        """Generate red flags."""
        red_flags = []
        ...
        if pattern_type == "coordinated_insider_trading":
            red_flags.extend(["coordinated_activity", "potential_conspiracy"])

        if pattern_type == "executive_trading_pattern":
            red_flags.extend(["executive_level_involvement", "fiduciary_duty_concern"])
    ...
    def generate_complex_insider_network(
        self, network_size: int = 10, trade_count: int = 50, days_before_announcement: int = 60
    ) -> Tuple[Pattern, List[Any], List[Any]]:
        """
