...
class TBMLPatternGenerator(BaseGenerator[Pattern]):
    ...
    def generate(
        ...
        pattern = Pattern(
            ...
            metadata={
                ...
                "average_document_value": float(sum(d.total_amount for d in documents) / len(documents)) if documents else 0,
                "average_transaction_value": float(total_value / len(transactions)) if transactions else 0,
                "high_risk_route": random.choice(self.high_risk_routes) if random.random() < 0.5 else None,
                "price_variance_percentage": random.uniform(50, 500) if pattern_type in ["over_invoicing", "under_invoicing"] else 0
            }
        )
    ...
    def _generate_pattern_documents(
        ...
        for i in range(document_count):
            ...
            if pattern_type == "over_invoicing":
                ...
                doc.total_amount = sum((Decimal(str(item["amount"])) for item in doc.line_items), Decimal("0.00"))
                ...
                if i > 0:
                    doc.metadata["shipment_id"] = documents[0].metadata.get("shipment_id", f"SHIP-{self.faker.uuid4()[:8]}")
                else:
    ...
    def _generate_pattern_transactions(
        ...
        for i in range(transaction_count):
            ...
            txn = self.txn_gen.generate(
                from_account_id=f"ACC-{from_entity[-8:]}",
                to_account_id=f"ACC-{to_entity[-8:]}"
            )
    ...
    def _generate_red_flags(
        ...
        if pattern_type == "over_invoicing":
            red_flags.extend([
                "significant_price_inflation",
                "money_movement_suspected"
            ])
            
        elif pattern_type == "under_invoicing":
            red_flags.extend([
                "customs_duty_evasion_suspected",
                "tax_evasion_indicator"
            ])
            
        elif pattern_type == "phantom_shipping":
            red_flags.extend([
                "no_physical_goods_movement",
                "paper_transaction_only"
            ])
            
        elif pattern_type == "multiple_invoicing":
            red_flags.extend([
                "invoice_fraud_suspected",
                "duplicate_billing"
            ])
