...
class AMLDataGenerator:
    ...
    def __init__(self, num_normal_customers=50, num_mule_accounts=10, 
                 num_beneficiaries=2, time_window_hours=24):
        """
    ...
    def create_account(self, person_id: str, account_type='checking', 
                       is_beneficiary=False) -> Dict:
        """Create an account entity"""
        account = {
            'account_id': self.generate_account_id(),
            'account_type': account_type,
            'account_status': 'active',
            'balance': random.uniform(50000, 500000) if is_beneficiary else random.uniform(1000, 50000),
            'open_date': fake.date_between(start_date='-5y', end_date='-1y').isoformat(),
            'owner_person_id': person_id
        }
        return account
    
    def create_transaction(self, from_account_id: str, to_account_id: str,
                          amount: float, timestamp: datetime, 
                          is_structuring=False) -> Dict:
        """Create a transaction entity"""
        transaction = {
            'transaction_id': self.generate_transaction_id(),
            'amount': round(amount, 2),
            'transaction_type': 'deposit' if is_structuring else random.choice(['deposit', 'withdrawal', 'transfer']),
    ...
    def generate_structuring_pattern(self, beneficiary_account_id: str,
    ...
    def generate_normal_transactions(self, accounts: List[Dict],
    ...
    def generate_all_data(self) -> Dict:
        ...
        # Summary statistics
        structuring_txns = [t for t in self.transactions if t.get('suspicious_pattern') == 'structuring']
