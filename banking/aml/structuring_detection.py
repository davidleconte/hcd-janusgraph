...
class StructuringDetector:
    ...
    def detect_smurfing(
        self, account_id: str, time_window_hours: int = 24, min_transactions: int = 3
        ...
        try:
            ...
            if len(transactions) < min_transactions:
                logger.info(
                    "No smurfing pattern detected (only %s transactions)", len(transactions)
                )
                return []

            # Analyze pattern
            pattern = self._analyze_smurfing_pattern(account_id, transactions, time_window_hours)

            if pattern:
                logger.warning(
                    "⚠️  SMURFING DETECTED: %s transactions, " "total $%s",
                    len(transactions),
    ...
    def detect_layering(
        self, account_ids: List[str], time_window_hours: int = 48
    ) -> List[StructuringPattern]:
        """
    ...
    def detect_network_structuring(
        self, seed_account_id: str, max_hops: int = 3, time_window_hours: int = 24
    ) -> List[StructuringPattern]:
        """
        ...
        try:
            ...
            for account_id in network_accounts:
                txs = (
                    ...
                    .has(
                        "amount",
                        P.between(float(self.suspicious_threshold), float(self.ctr_threshold)),
                    )
            ...
            if len(suspicious_txs) >= self.MIN_TRANSACTIONS_FOR_PATTERN * len(network_accounts) / 2:
                ...
                if pattern:
                    logger.warning(
                        "⚠️  NETWORK STRUCTURING DETECTED: %s accounts, " "%s transactions",
                        len(network_accounts),
                        len(suspicious_txs),
                    )
                    return [pattern]
    ...
    def _analyze_smurfing_pattern(
        self, account_id: str, transactions: List[Dict], time_window_hours: int
        ...
        if below_threshold_count >= len(amounts) * 0.8:
            indicators.append(
                f"{below_threshold_count} transactions just below ${self.ctr_threshold}"
            )
            confidence += 0.3
    ...
    def _analyze_network_pattern(
        self, account_ids: List[str], transactions: List[Dict], time_window_hours: int
    ) -> Optional[StructuringPattern]:
    ...
    def generate_alert(self, patterns: List[StructuringPattern]) -> Optional[StructuringAlert]:
        """
