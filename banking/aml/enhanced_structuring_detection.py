...
class EnhancedStructuringDetector:
        ...
        try:
            ...
            for result in results:
                ...
                if account_id not in account_txs:
                    account_txs[account_id] = {"account": account_data, "transactions": []}
                ...
                if total > threshold_amount:
                    ...
                    if person_results:
                        ...
                        risk_score = self._calculate_risk_score(
                            total_amount=total, tx_count=len(txs), threshold=threshold_amount
                        )
    ...
    def detect_semantic_patterns(
        ...
        """
        logger.info(
            "Detecting semantic structuring patterns (similarity>=%s, window=%sh)...",
            min_similarity,
            time_window_hours,
        )
        ...
        try:
            ...
            recent_txns = (
                ...
                .by(
                    __.in_("MADE_TRANSACTION")
                    .in_("OWNS_ACCOUNT")
                    .coalesce(__.values("full_name"), __.values("company_name"))
                    .fold()
                )
            ...
            if len(recent_txns) < min_cluster_size:
                logger.info(
                    "Only %s recent transactions found - insufficient for pattern detection",
                    len(recent_txns),
                )
                return patterns
            ...
            for i, txn in enumerate(tx_data):
                ...
                if total_amount >= self.STRUCTURING_THRESHOLD:
                    pattern_id = (
                        f"SEM-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}-{len(patterns)}"
                    )
                    ...
                    if isinstance(primary_person_name, list):
                        primary_person_name = (
                            primary_person_name[0] if primary_person_name else "Unknown"
                        )
                    ...
                    risk_score = min(
                        1.0,
                        0.5
                        + 0.2 * (len(cluster_txns) / 10.0)  # Base risk for semantic similarity
                        + 0.3  # More transactions = higher risk
                        * (total_amount / (self.STRUCTURING_THRESHOLD * 3)),  # Amount factor
                    )
    ...
    def _calculate_risk_score(self, total_amount: float, tx_count: int, threshold: float) -> float:
        """
    ...
    def detect_structuring(
        ...
        near_threshold_count = sum(
            1 for t in transactions if 9000 <= t.get("amount", 0) < self.STRUCTURING_THRESHOLD
        )
        ...
        is_structuring = (
            near_threshold_count >= self.MIN_TRANSACTIONS
    ...
    def detect_multi_account_structuring(
        ...
        coordinated_accounts = [
            acc_id for acc_id, result in account_results.items() if result["is_structuring"]
        ]
        ...
        if is_coordinated:
            ...
            risk_score = (
                max(r["risk_score"] for r in account_results.values()) if account_results else 0
            )
            risk_level = (
                "medium" if any(r["is_structuring"] for r in account_results.values()) else "low"
            )
    ...
    def analyze_amount_clustering(
        ...
        is_clustered = (
            ...
            and len(near_threshold) >= self.MIN_TRANSACTIONS  # Very tight cluster
            and 8500 <= avg_amount < self.STRUCTURING_THRESHOLD
        )
    ...
    def generate_report(
        self, patterns: List[StructuringPattern], output_format: str = "text"
    ) -> str:
        """
