...
class HCDValidator:
    ...
    def validate_janusgraph_tables(self, keyspace: str = "janusgraph") -> bool:
        ...
        try:
            ...
            if missing_tables:
                print(
                    f"   ⚠️  Missing tables (will be created by JanusGraph): {', '.join(missing_tables)}"
                )
...
def main():
    ...
    parser.add_argument(
        "--hosts",
        default=os.getenv("HCD_HOST", "localhost"),
        help="Comma-separated list of HCD hosts (default: localhost)",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.getenv("HCD_PORT", "9042")),
        help="HCD CQL port (default: 9042)",
    )
    parser.add_argument(
        "--username", default=os.getenv("HCD_USERNAME"), help="HCD username (optional)"
    )
    parser.add_argument(
        "--password", default=os.getenv("HCD_PASSWORD"), help="HCD password (optional)"
    )
    parser.add_argument(
        "--keyspace", default="janusgraph", help="Keyspace to validate (default: janusgraph)"
    )
    parser.add_argument(
        "--datacenter", default="datacenter1", help="Local datacenter name (default: datacenter1)"
    )
