...
DEMO_STREAMING_DETERMINISTIC_IDS="${DEMO_STREAMING_DETERMINISTIC_IDS:-${DEMO_DETERMINISTIC_MODE}}"
DEMO_REQUIRE_EXISTING_BASELINE="${DEMO_REQUIRE_EXISTING_BASELINE:-0}"
...
export DEMO_STREAMING_DETERMINISTIC_IDS="$DEMO_STREAMING_DETERMINISTIC_IDS"
export DEMO_REQUIRE_EXISTING_BASELINE="$DEMO_REQUIRE_EXISTING_BASELINE"
...
SUMMARY_FILE="${REPORT_DIR}/pipeline_summary.txt"
{
    ...
    echo "Steps: reset, preflight, isolation, deploy, service-boot, runtime-contracts, runtime-package-fingerprint, notebook-determinism-sweep, service-snapshot, graph-seed, notebook-prereq-proof, notebooks, notebook-output-validation, data-generators, manifest, determinism, drift-detection, kpi-drift-detection, kpi-trend-aggregation, kpi-evidence-bundling"
    ...
    echo "DEMO_STREAMING_DETERMINISTIC_IDS=${DEMO_STREAMING_DETERMINISTIC_IDS}"
    echo "DEMO_REQUIRE_EXISTING_BASELINE=${DEMO_REQUIRE_EXISTING_BASELINE}"
...
if [[ "$DRY_RUN" == "false" ]]; then
    ...
    if [[ "$DEMO_DETERMINISTIC_MODE" == "1" ]]; then
        ...
        # KPI trend aggregation for cross-run governance visibility (FR-043)
        run_cmd "KPI Trend Aggregation" \
            "python3 scripts/testing/aggregate_kpi_trends.py" \
            "${REPORT_DIR}/kpi_trends.log" \
            python3 scripts/testing/aggregate_kpi_trends.py

        # Governance evidence bundling for audit handoff (FR-044)
        run_cmd "KPI Evidence Bundling" \
            "python3 scripts/testing/bundle_governance_evidence.py" \
            "${REPORT_DIR}/kpi_bundle.log" \
            python3 scripts/testing/bundle_governance_evidence.py
    fi
fi
