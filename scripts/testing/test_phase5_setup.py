...
def test_vector_search():
    ...
    try:
        ...
        test_names = ["John Smith", "Jane Doe", "Bob Johnson", "Alice Williams", "Charlie Brown"]

        documents = []
        for i, name in enumerate(test_names):
            embedding = generator.encode_for_search(name)
            documents.append({"id": str(i + 1), "embedding": embedding, "name": name})
        ...
        results = client.search(index_name=index_name, query_embedding=query_embedding, k=3)
