...
if ! podman ps | grep -q vault-server; then
...
for i in {1..30}; do
    if podman exec vault-server vault status >/dev/null 2>&1 || [ $? -eq 2 ]; then
        break
    fi
    sleep 1
done
...
if podman exec vault-server vault status 2>&1 | grep -q "Initialized.*true"; then
...
INIT_OUTPUT=$(podman exec vault-server vault operator init -key-shares=5 -key-threshold=3 -format=json)
...
podman exec vault-server vault operator unseal "$UNSEAL_KEY_1" >/dev/null
podman exec vault-server vault operator unseal "$UNSEAL_KEY_2" >/dev/null
podman exec vault-server vault operator unseal "$UNSEAL_KEY_3" >/dev/null
...
podman exec -e VAULT_TOKEN=$ROOT_TOKEN vault-server vault secrets enable -path=janusgraph kv-v2
...
podman exec -e VAULT_TOKEN=$ROOT_TOKEN vault-server vault kv put janusgraph/admin \
    username=admin \
    password="$ADMIN_PASSWORD"

podman exec -e VAULT_TOKEN=$ROOT_TOKEN vault-server vault kv put janusgraph/hcd \
    username=cassandra \
    password="$HCD_PASSWORD"

podman exec -e VAULT_TOKEN=$ROOT_TOKEN vault-server vault kv put janusgraph/grafana \
    username=admin \
    password="$GRAFANA_PASSWORD"

podman exec -e VAULT_TOKEN=$ROOT_TOKEN vault-server vault kv put janusgraph/opensearch \
    username=admin \
    password="$ADMIN_PASSWORD"
...
podman exec -e VAULT_TOKEN=$ROOT_TOKEN vault-server vault policy write janusgraph-policy /tmp/janusgraph-policy.hcl
...
APP_TOKEN=$(podman exec -e VAULT_TOKEN=$ROOT_TOKEN vault-server vault token create \
