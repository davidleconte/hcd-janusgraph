        ...
        if self._is_public(node.name):
            ...
            if self._has_docstring(node):
                ...
                if self.current_class:
                    self.result.missing_docs.append(f"Method: {self.current_class}.{node.name}")
                else:
...
def generate_report(results: List[DocCoverageResult]) -> Dict:
    ...
    for r in results:
        for item in r.missing_docs[:5]:  # Limit per file
            all_missing.append({"file": r.file_path, "item": item})
...
def main():
    ...
    parser.add_argument(
        "--threshold", type=float, default=80.0, help="Minimum coverage threshold (default: 80)"
    )
    parser.add_argument(
        "--path", type=str, default=".", help="Path to analyze (default: current directory)"
    )
