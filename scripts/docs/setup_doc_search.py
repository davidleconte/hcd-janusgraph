            ...
            if current_content:
                ...
                for i, chunk in enumerate(_chunk_text(section_text)):
                    chunk_id = hashlib.md5(
                        f"{file_path}:{current_section}:{i}".encode(), usedforsecurity=False
                    ).hexdigest()[:12]
                    yield {
                        "content": chunk,
...
def index_documents(client: OpenSearch, model: SentenceTransformer, docs_path: Path) -> int:
    ...
    md_files = [f for f in md_files if not any(ex in str(f) for ex in exclude)]
    ...
    for md_file in md_files:
        try:
            ...
            for chunk, embedding in zip(chunks, embeddings):
                doc = {**chunk, "embedding": embedding.tolist()}
                client.index(index=DOC_INDEX, id=chunk["chunk_id"], body=doc)
                indexed += 1
...
def search_docs(
    client: OpenSearch, model: SentenceTransformer, query: str, top_k: int = 5
) -> List[Dict]:
