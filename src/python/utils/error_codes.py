...
class AppException(Exception):
    ...
    def __init__(self, error_code: str, details: Optional[str] = None, context: Optional[Dict[str, Any]] = None):
        self.error = ERROR_CODES.get(error_code)
        if not self.error:
            self.error = ErrorCode(error_code, ErrorCategory.INTERNAL, "Unknown error", 500)
        self.details = details
        self.context = context or {}
        super().__init__(f"[{self.error.code}] {self.error.message}: {details}" if details else f"[{self.error.code}] {self.error.message}")
...
class NotFoundError(AppException):
    ...
    def __init__(self, entity_type: str, entity_id: str):
        super().__init__("DB_003", f"{entity_type} '{entity_id}' not found", {"entity_type": entity_type, "entity_id": entity_id})
