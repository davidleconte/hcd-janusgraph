...
def get_credentials(
    ...
    logger.debug("%s authentication configured for user: %s", service_name, username)

    return username, password


def validate_ssl_config(use_ssl: bool, verify_certs: bool, ca_certs: Optional[str] = None) -> None:
    """
    ...
    if ca_certs and not use_ssl:
        logger.warning(
            "CA certificates provided but SSL is disabled. " "CA certificates will be ignored."
        )
