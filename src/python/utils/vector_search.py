...
class VectorSearchClient:
    ...
    def create_vector_index(
        ...
        logger.info(
            "Created vector index: %s (dim=%s, engine=%s)", index_name, vector_dimension, engine
        )
    ...
    def index_document(
        ...
        response = self.client.index(index=index_name, id=doc_id, body=doc, refresh=True)
    ...
    def search(
        ...
        if filters:
            search_body["query"] = {
                "bool": {
                    "must": [{"knn": {vector_field: {"vector": query_vector, "k": k}}}],
                    "filter": filters,
                }
            }
        ...
        for hit in response["hits"]["hits"]:
            result = {"id": hit["_id"], "score": hit["_score"], "source": hit["_source"]}
...
if __name__ == "__main__":
    ...
    client.create_vector_index(
        index_name=index_name, vector_dimension=384, additional_fields={"text": {"type": "text"}}
    )

    # Index test documents
    print("\nIndexing test documents...")
    test_docs = [
        {"id": "1", "embedding": np.random.rand(384), "text": "John Smith"},
        {"id": "2", "embedding": np.random.rand(384), "text": "Jane Doe"},
    ]
    ...
    for result in results:
        print(
            f"  - ID: {result['id']}, Score: {result['score']:.4f}, Text: {result['source']['text']}"
        )
