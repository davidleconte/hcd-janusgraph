...
class QueryProfiler:
    ...
    def __init__(
        self, slow_query_threshold_ms: float = 1000.0, expensive_query_threshold_scans: int = 10000
    ):
        """
    ...
    def get_slow_queries(self, limit: int = 10) -> List[QueryStatistics]:
        ...
        """
        sorted_stats = sorted(
            self.query_statistics.values(), key=lambda s: s.avg_time_ms, reverse=True
        )
    ...
    def get_frequent_queries(self, limit: int = 10) -> List[QueryStatistics]:
        ...
        """
        sorted_stats = sorted(
            self.query_statistics.values(), key=lambda s: s.execution_count, reverse=True
        )
        return sorted_stats[:limit]
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        if total_queries > 0:
            avg_execution_time = statistics.mean(m.execution_time_ms for m in self.query_metrics)
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        high_scan_queries = [
            s
            for s in self.query_statistics.values()
        ...
        large_result_queries = [
            s for s in self.query_statistics.values() if s.max_result_count > 1000
        ]
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        result, metrics = profiler.profile_query(query, execute_query, query, user_id=f"user{i}")
