...
class PerformanceBenchmark:
    ...
    def compare_benchmarks(
        ...
        """
        avg_diff_percent = (
            (current.avg_time_ms - baseline.avg_time_ms) / baseline.avg_time_ms * 100
        )
        p95_diff_percent = (
            (current.p95_time_ms - baseline.p95_time_ms) / baseline.p95_time_ms * 100
        )
        ...
        is_regression = (
            avg_diff_percent > threshold_percent or
            p95_diff_percent > threshold_percent or
            throughput_diff_percent < -threshold_percent
        )
        ...
        data = {
            ...
            'summary': {
                ...
                'avg_throughput_qps': statistics.mean(
                    r.throughput_qps for r in self.results
