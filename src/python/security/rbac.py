...
class RBACManager:
    ...
    def check_permission(
        ...
        if resource_type and resource_id:
            if not self._check_resource_access(user, resource_type, resource_id):
                logger.debug(
                    "User %s denied access to resource %s:%s",
                    user_id,
                    resource_type.value,
                    resource_id,
                )
    ...
    def _check_resource_access(
        self, user: User, resource_type: ResourceType, resource_id: str
    ) -> bool:
        """
    ...
    def evaluate_access(self, request: AccessRequest) -> AccessDecision:
        ...
        if not self.get_user(request.user.user_id):
            return AccessDecision(
                allowed=False, reason="User not found", evaluated_policies=evaluated_policies
            )

        # Check permission
        has_permission = self.check_permission(
            request.user.user_id, request.action, request.resource_type, request.resource_id
        )
        ...
        return AccessDecision(
            allowed=True, reason="Access granted", evaluated_policies=evaluated_policies
        )
