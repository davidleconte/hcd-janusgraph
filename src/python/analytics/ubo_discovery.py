...
class UBODiscovery:
    ...
    HIGH_RISK_JURISDICTIONS = {
        'VG', 'KY', 'PA', 'BZ', 'SC',  # Tax havens
        'RU', 'BY', 'IR', 'KP', 'SY'   # Sanctioned
    }
    ...
    def find_ubos_for_company(
        ...
        for owner in direct_owners:
            if owner['ownership_percentage'] >= self.ownership_threshold:
                ...
                ownership_chains.append([OwnershipLink(
        ...
        risk_score = self._calculate_risk_score(
            ubos, ownership_chains, high_risk_indicators
        )
    ...
    def _find_indirect_owners(
        ...
        try:
            ...
            results = self.g.V().has('company_id', company_id) \
                .repeat(
                    __.inE('owns_company', 'beneficial_owner')
                    .outV()
                    .simplePath()
                ) \
                .until(
        ...
        for obj in objects:
            if isinstance(obj, dict):
                ...
                if flat.get('person_id'):
                    chain.append(OwnershipLink(
                        entity_id=flat['person_id'],
                        entity_type='person',
                        entity_name=flat.get('full_name', f"{flat.get('first_name', '')} {flat.get('last_name', '')}"),
    ...
    def _calculate_risk_score(
        self,
        ubos: List[Dict],
        chains: List[List[OwnershipLink]],
        indicators: List[str]
    ) -> float:
        """
    ...
    def get_ownership_network(
        ...
        try:
            ...
            results = self.g.V().has(id_field, entity_id) \
                .repeat(
            ...
            for path in results:
                ...
                for i, obj in enumerate(objects):
                    if isinstance(obj, dict):
                        flat = self._flatten_value_map(obj)
                        node_id = flat.get('person_id') or flat.get('company_id') or flat.get('account_id')
                        ...
                        if node_id and node_id not in visited:
                            ...
                            nodes.append({
                                ...
                                'name': flat.get('full_name') or flat.get('legal_name') or flat.get('account_number', 'Unknown'),
                                'properties': flat
                            })
            ...
            for node in nodes:
                ...
                for id_field in ['person_id', 'company_id', 'account_id']:
                    try:
                        edge_results = self.g.V().has(id_field, node_id) \
