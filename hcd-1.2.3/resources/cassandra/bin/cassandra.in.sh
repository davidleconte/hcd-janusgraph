...
if [ "$short" = "11" ]  ; then
    ...
    if [ -z "$CASSANDRA_JDK_UNSUPPORTED" ] ; then
        echo "Unsupported JDK $short. Supported is JDK 11."
        echo "If you would like to test with JDK 17 set CASSANDRA_JDK_UNSUPPORTED to any value (for example, CASSANDRA_JDK_UNSUPPORTED=true). Unset the parameter for default behavior"
        exit 1;
    else
        echo "######################################################################"
        echo "Warning! You are using JDK $short. This HCD version only supports JDK 11."
        echo "######################################################################"
    fi
else
    echo "HCD requires Java 11 or 17 (experimental). Java $JVM_VERSION is not supported."
    exit 1;
fi
